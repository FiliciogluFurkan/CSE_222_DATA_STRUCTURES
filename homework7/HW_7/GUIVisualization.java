...
public class GUIVisualization extends JFrame {
    ...
    public GUIVisualization(String plotType, Map<Integer,Long> data,String command) {
       ...
       //Very very important
        boolean average = true; //it shows average graph if we dont wawnt to show detailed graph,we can change it to false
       //you can change this part manually to see all the data pointsmak

        int counter = 0;
        long tot = 0;

        // therefore int i = 10 stars from 10 not 0, and that means the first 10 data are ignored
        if (average) {
            for (int i = 10; i < data.size() ; i++) {
                    ...
                    if (counter < 15){
                        ...
                        counter = 0;
                        dataPointsX.add(i);
                        dataPointsY.add((tot / 15));
                        tot = 0;
                    }
            }
        } else { //it shows not detailed graph
            for (int i = 10; i < data.size(); i++) {
                dataPointsX.add(i);
                dataPointsY.add(data.get(i));
            }
        }

        setTitle("Performance Graph Visualization - "+command); // Set the title of the window
        setSize(1500, 900); // Set the size of the window
    ...
    private void drawGraph(Graphics g) {
        ...
        int labelPadding = 15; // Padding for labels
        ...
        int numberYDivisions = 15; // Number of divisions for the y-axis
        ...
        if (plotType.equals("line")) {
            ...
            for (int i = 0; i < dataPointsX.size(); i++) {
                ...
                g2.fillOval(x - 3, y - 3, 5, 5); // Draw data point as a small circle
            }
        }
